...
/**
 * Métricas para el dashboard de inicio: KPIs de llamadas del intervalo
 * elegido (hoy/ayer/semana/semana pasada/mes — antes solo "hoy"), y
 * satisfacción evaluada por el bot (últimos 30 días fijo, ya que día a día
 * hay muy pocas llamadas para que el % diga algo).
 */
export async function resumenRoutes(app: FastifyInstance) {
  app.get<{ Querystring: { empresaId?: string; colaId?: string; rango?: string } }>(
    "/api/resumen",
    async (req, reply) => {
      ...
      const [llamadas, campanas, satisfaccion, porDia] = await Promise.all([
        pool.query(
          `SELECT
             COUNT(*) FILTER (WHERE iniciada_en >= ${desdeSQL} AND iniciada_en < ${hastaSQL}) AS llamadas_hoy,
             COUNT(*) FILTER (WHERE estado = 'en_curso') AS llamadas_activas,
             COUNT(*) FILTER (WHERE transferida AND iniciada_en >= ${desdeSQL} AND iniciada_en < ${hastaSQL}) AS transferidas_hoy,
             COUNT(*) FILTER (WHERE direccion = 'entrante' AND iniciada_en >= ${desdeSQL} AND iniciada_en < ${hastaSQL}) AS entrantes_hoy,
             COUNT(*) FILTER (WHERE direccion = 'saliente' AND iniciada_en >= ${desdeSQL} AND iniciada_en < ${hastaSQL}) AS salientes_hoy,
             COUNT(*) FILTER (WHERE estado = 'completada' AND iniciada_en >= ${desdeSQL} AND iniciada_en < ${hastaSQL}) AS completadas_hoy,
             COUNT(*) FILTER (WHERE estado = 'fallida' AND iniciada_en >= ${desdeSQL} AND iniciada_en < ${hastaSQL}) AS fallidas_hoy
